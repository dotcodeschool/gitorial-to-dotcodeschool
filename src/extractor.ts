...
export async function extractGitorialData(
  repoPath: string,
  branch: string
): Promise<GitorialData> {
  ...
  if (branch === "master" && hasStepsDir) {
    console.log(
      "Detected steps directory structure. Extracting data from steps..."
    );
  ...
  const commits: GitCommit[] = gitLogOutput
    .trim()
    .split("\n")
    .map((line) => {
  ...
  for (const commit of commits) {
    ...
    if (message.startsWith("section:")) {
      ...
      let readmeContent = "";
      try {
        readmeContent = execSync(`git show ${hash}:README.md`, {
          cwd: repoPath,
        }).toString();
      } catch (error) {
        console.warn(`Warning: Could not find README.md in commit ${hash}`);
      }
  ...
  for (const template of templateCommits) {
    ...
    const matchingSolution = solutionCommits.find(
      (solution) => solution.message.trim() === template.message.trim()
    );
  ...
  allLessons.sort((a, b) => {
    const aHash =
      a.type === "template-solution" ? a.templateHash : a.sourceHash;
    const bHash =
      b.type === "template-solution" ? b.templateHash : b.sourceHash;
  ...
  for (const lesson of allLessons) {
    ...
    for (let i = currentSectionIndex + 1; i < sections.length; i++) {
      const sectionCommitIndex = commits.findIndex(
        (c) => c.hash === sections[i].hash
      );
  ...
  if (allLessons.length === 0 && hasStepsDir) {
    console.log(
      "No lessons found in commits. Trying to extract from steps directory..."
    );
    return await extractDataFromSteps(repoPath);
  }
    ...
    for (const lesson of section.lessons) {
      if (lesson.type === "template-solution") {
        ...
        if (lesson.templateHash) {
          const templateContent = await getCommitContent(
            repoPath,
            lesson.templateHash
          );
          lesson.templateContent = templateContent;
          // Update title if README title is available
          if (templateContent.readmeTitle) {
            lesson.title = templateContent.readmeTitle;
          }
        }

        // Get solution content if available
        if (lesson.solutionHash) {
          const solutionContent = await getCommitContent(
            repoPath,
            lesson.solutionHash
          );
          lesson.solutionContent = solutionContent;
        }
      } else if (lesson.type === "source" && lesson.sourceHash) {
        // Get source content
        const sourceContent = await getCommitContent(
          repoPath,
          lesson.sourceHash
        );
        lesson.sourceContent = sourceContent;
        // Update title if README title is available
        if (sourceContent.readmeTitle) {
          lesson.title = sourceContent.readmeTitle;
        }
      }
    }
  }
...
async function extractDataFromSteps(repoPath: string): Promise<GitorialData> {
  ...
  for (const stepDir of sortedStepDirs) {
    ...
    try {
      ...
      throw new Error(
        "No metadata file or invalid JSON, continuing without it"
      );
    ...
    // Extract title from README if available, otherwise use commit message or default
    let title = `Step ${stepDir}`;
    const titleMatch = readmeContent.match(/^#\s+(.+)$/m);
    if (titleMatch) {
      title = titleMatch[1].trim();
    } else if (commitMessage) {
      // Only use commit message title if no README title found
      if (commitMessage.startsWith("section:")) {
        title = commitMessage.substring("section:".length).trim();
      } else if (commitMessage.startsWith("template:")) {
        title = commitMessage.substring("template:".length).trim();
      } else if (commitMessage.startsWith("solution:")) {
        title = commitMessage.substring("solution:".length).trim();
      } else if (commitMessage.startsWith("action:")) {
        title = commitMessage.substring("action:".length).trim();
      }
    }
  ...
  for (const sectionStep of sectionSteps) {
    // Extract title from README if available
    let title = sectionStep.title;
    let readmeContent = sectionStep.readmeContent;
    ...
    const section: Section = {
      ...
      readmeContent, // Store the complete README content
    };
  ...
  if (sections.length === 0) {
    throw new Error(
      'No sections found in the gitorial. The gitorial format must have at least one section commit with the prefix "section:"'
    );
  }
  ...
  for (const template of templateSteps) {
    ...
    const matchingSolution = solutionSteps.find(
      (solution) => parseInt(solution.dir) === templateStepNumber + 1
    );
  ...
  const uncategorizedSteps = allSteps.filter(
    (step) => !categorizedStepDirs.has(step.dir)
  );

  const uncategorizedLessons: SourceLesson[] = uncategorizedSteps.map(
    (step) => ({
      title: step.title,
    ...
    for (const lesson of section.lessons) {
      if (lesson.type === "template-solution") {
        ...
        if (lesson.templateStep) {
          lesson.templateContent = await getDirectoryContent(
            lesson.templateStep.path
          );
        }

        // Get solution content if available
        if (lesson.solutionStep) {
          lesson.solutionContent = await getDirectoryContent(
            lesson.solutionStep.path
          );
        }
      } else if (lesson.type === "source" && lesson.sourceStep) {
        // Get source content
        lesson.sourceContent = await getDirectoryContent(
          lesson.sourceStep.path
        );
      }
    }
  }
...
async function getCommitContent(
  repoPath: string,
  commitHash: string
): Promise<LessonContent & { readmeTitle?: string }> {
  console.log(
    `Extracting content from commit ${commitHash.substring(0, 8)}...`
  );
  ...
  for (const file of files) {
    try {
      ...
      console.warn(
        `Warning: Could not extract content for file ${file} in commit ${commitHash}`
      );
    }
  }

  const result: LessonContent & { readmeTitle?: string } = content;
  if (readmeTitle) {
    result.readmeTitle = readmeTitle;
  }
  return result;
}
